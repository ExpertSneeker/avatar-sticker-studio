...
import { ArrowLeft, Copy, Plus, RefreshCw, Search, Users } from 'lucide-react'
...
export function Accounts() {
  ...
  if(selected)return <div className="accounts-page"><button className="text-button accounts-back" onClick={()=>{setSelected(null);refresh()}}><ArrowLeft size={16}/>返回账号管理</button><div className="page-heading"><div><h1>{selected.display_name}</h1><p>{selected.username} · {selected.role==='admin'?'管理员':'成员'} · {selected.active===false?'已停用':'启用中'}</p></div><label className="period-filter">流水范围<select value={days} onChange={e=>setDays(e.target.value)}>{periods.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label></div><CreditsPanel userId={selected.id} days={days}/></div>

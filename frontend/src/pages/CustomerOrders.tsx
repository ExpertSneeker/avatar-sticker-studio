...
// Rows render in pages; filters, counts and "全选当前订单" still cover every matching order.
const PAGE_SIZE=50
...
export function CustomerOrders({user,library,createRequest=0,onCreateHandled,scope='active'}:{user:User;library:CustomerLibrary;createRequest?:number;onCreateHandled?:()=>void;scope?:'active'|'history'}){
  ...
  const [search,setSearch]=useState(''),[state,setState]=useState('all'),[shop,setShop]=useState('all'),[owner,setOwner]=useState('all'),[start,setStart]=useState(''),[end,setEnd]=useState('')
  ...
  const [remarkEdit,setRemarkEdit]=useState(false),[remark,setRemark]=useState('')
  const session=useRef(new AbortController()),lock=useRef(false),mutations=useRef(new OrderMutations()),bulkToken=useRef('')
  // The list polls lightweight summaries; the open order keeps its own full record (avatars and slots).
  const [detail,setDetail]=useState<CustomerOrder|null>(null),selectedRef=useRef<string|null>(null),detailRef=useRef<CustomerOrder|null>(null)
  selectedRef.current=selected;detailRef.current=detail
  const loadDetail=useCallback(async(id:string)=>{const controller=session.current;try{const next=await api<CustomerOrder>('/customer-orders/'+encodeURIComponent(id),{signal:controller.signal});if(!controller.signal.aborted&&selectedRef.current===id)setDetail(prev=>prev&&prev.id===id&&prev.version>next.version?prev:next)}catch(e){if(!controller.signal.aborted)setError((e as Error).message)}},[])
  const history=scope==='history'
  const visibleOrders=history?orders:orders.filter(order=>order.state==='draft'||order.state==='review')
  const refresh=useCallback(async()=>{const controller=session.current;try{const next=await api<CustomerOrder[]>('/customer-orders?summary=1',{signal:controller.signal});if(!controller.signal.aborted){setOrders(prev=>next.map(order=>{const existing=prev.find(o=>o.id===order.id);return existing&&existing.version>order.version?existing:order}));setError('');const open=next.find(order=>order.id===selectedRef.current),current=detailRef.current;if(open&&current&&current.id===open.id&&open.version>current.version)void loadDetail(open.id)}}catch(e){if(!controller.signal.aborted)setError((e as Error).message)}finally{if(!controller.signal.aborted)setLoading(false)}},[loadDetail])
  useEffect(()=>{const controller=new AbortController();session.current=controller;void refresh();const stop=visiblePolling(()=>{if(!lock.current)void refresh()},4000);return()=>{controller.abort();stop()}},[refresh])
  useEffect(()=>{if(createRequest){setCreateOpen(true);onCreateHandled?.()}},[createRequest])
  useEffect(()=>{setDetail(prev=>prev&&prev.id===selected?prev:null);if(selected)void loadDetail(selected)},[selected,loadDetail])
  const [shown,setShown]=useState(PAGE_SIZE)
  useEffect(()=>setShown(PAGE_SIZE),[search,state,shop,owner,start,end,scope])
  const dateError=!!(start&&end&&start>end)
  const filtered=visibleOrders.filter(order=>(state==='all'||order.state===state)&&(shop==='all'||(shop==='none'?!order.shop_id:order.shop_id===shop))&&(owner==='all'||order.owner===owner)&&order.order_number.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())&&(!start||new Date(order.created_at||'').getTime()>=new Date(start+'T00:00:00').getTime())&&(!end||new Date(order.created_at||'').getTime()<=new Date(end+'T23:59:59.999').getTime())&&!dateError)
  const row=orders.find(order=>order.id===selected),active=row&&detail&&detail.id===row.id?detail:undefined,selectedOrders=orders.filter(order=>checked.includes(order.id)),ready=selectedOrders.filter(order=>order.state==='submitted'&&order.delivery_ready)
  function accept(next:CustomerOrder){setOrders(prev=>prev.map(order=>order.id===next.id&&next.version>=order.version?next:order));setDetail(prev=>prev&&prev.id===next.id&&next.version>=prev.version?next:prev)}
  ...
  async function saveRemark(e:React.FormEvent){
    ...
    lock.current=true;setBusy(true);setError('')
    try{const next=await mutations.current.run<CustomerOrder>('staff','/customer-orders/'+encodeURIComponent(active.id),'/remark',active.version,{remark:remark.trim()});accept(next);setRemarkEdit(false);notice('卖家备注已更新')}
  ...
  const detailView=active?<><div className="customer-staff-actions">{controls(active)}</div><div className="customer-order-meta">店铺：{active.shop_name||'无关联店铺'}<br/>开单人：{active.owner_name} · 备注：{active.notes||'无'}<br/>买家备注：{active.buyer_memo||'无'}<br/>卖家备注：{active.platform_remark||'无'}{' '}<button type="button" className="text-button" disabled={busy} onClick={()=>{setRemark(active.platform_remark||'');setRemarkEdit(true)}}>修改</button><br/>当前水印：{active.watermark||active.owner_name}{active.delivery_ready?' · 打印文件就绪':''}{downloads[active.id]&&<p role="status">{downloads[active.id]}</p>}</div><CustomerWorkbench key={active.id} initial={active} mode="staff" library={library} onBack={()=>setSelected(null)} onChange={next=>accept({...active,...next,version:next.version??active.version})}/></>:null
  const list=<><div className="page-heading"><div><h1>{history?'历史订单':'客户订单'}</h1><p>{history?'查看全部订单，包含待制作、选图中、已提交和已取消的记录。':'开单后将订单号交给客户，也可进入订单代客户制作和选图。'}</p></div>{!history&&<div className="button-group"><a className="button" href="/guest" target="_blank" rel="noreferrer">客户入口</a><button className="button primary" onClick={()=>setCreateOpen(true)}><Plus size={16}/>开新订单</button></div>}</div>
    <div className="customer-order-filters"><label className="field">订单号<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="搜索订单号"/></label><label className="field">店铺<select aria-label="店铺" value={shop} onChange={e=>{setShop(e.target.value);setChecked([])}}><option value="all">全部店铺</option><option value="none">无关联店铺</option>{Array.from(new Map(orders.filter(order=>order.shop_id).map(order=>[order.shop_id!,order.shop_name||order.shop_id!])).entries()).map(([id,name])=><option key={id} value={id}>{name}</option>)}</select></label><label className="field">开单人<select value={owner} onChange={e=>setOwner(e.target.value)}><option value="all">全部开单人</option>{Array.from(new Map(visibleOrders.map(order=>[order.owner,order.owner_name])).entries()).map(([id,name])=><option key={id} value={id}>{name}</option>)}</select></label><label className="field">订单状态<select value={state} onChange={e=>{setState(e.target.value);setChecked([])}}><option value="all">{history?'全部订单':'全部在制订单'}</option>{(history?Object.entries(stateLabels):Object.entries(stateLabels).filter(([value])=>value==='draft'||value==='review')).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label className="field">开始日期<input type="date" value={start} onChange={e=>setStart(e.target.value)}/></label><label className="field">结束日期<input type="date" value={end} min={start} onChange={e=>setEnd(e.target.value)}/></label><button className="button" onClick={()=>void refresh()} disabled={busy}><RefreshCw size={16}/>刷新</button></div>{dateError&&<p className="error-banner">结束日期不能早于开始日期</p>}
    ...
    {loading?<Spinner/>:filtered.length?<><div className="customer-order-list">{filtered.slice(0,shown).map(order=><article key={order.id} className="customer-order-row"><input type="checkbox" aria-label={'选择订单 '+order.order_number} checked={checked.includes(order.id)} onChange={e=>setChecked(prev=>e.target.checked?[...prev,order.id]:prev.filter(id=>id!==order.id))}/>{history&&<span className="customer-order-thumb">{order.preview_url?<img src={order.preview_url+'&size=160'} alt="订单预览" loading="lazy" decoding="async"/>:<span className="customer-order-thumb-empty">无预览</span>}</span>}<button className="customer-order-open" onClick={()=>setSelected(order.id)}><strong>{order.order_number}</strong><span>店铺：{order.shop_name||'无关联店铺'}</span><span>{order.owner_name} · 最终提交 {order.final_count} 张 · 可预览 {order.generation_limit} 张</span>{order.notes&&<span>备注：{order.notes}</span>}{order.buyer_memo&&<span>买家备注：{order.buyer_memo}</span>}{order.platform_remark&&<span>卖家备注：{order.platform_remark}</span>}<small>{submissionDate(order.created_at||'')}</small>{downloads[order.id]&&<small role="status">{downloads[order.id]}</small>}</button><span className="customer-order-status">{stateLabels[order.state]}{order.state==='submitted'&&!order.delivery_ready?' · 排版中':''}</span><div className="customer-order-actions"><button className="button" onClick={()=>setSelected(order.id)}>{order.state==='draft'||order.state==='review'?'进入 / 代操作':'查看订单'}</button>{controls(order)}</div></article>)}</div>{filtered.length>shown&&<button className="button customer-order-more" onClick={()=>setShown(value=>value+PAGE_SIZE)}>显示更多（还有 {filtered.length-shown} 个）</button>}</>:<Empty title="没有匹配的订单" description={history?'调整筛选条件即可查看其他订单。':'调整筛选条件，或开一个新订单。'}/>}</>
  return <div className="customer-orders">{list}{row&&<Modal title={'订单 '+row.order_number} onClose={()=>{if(!busy)setSelected(null)}} wide>{active?detailView:<Spinner/>}</Modal>}
    ...
    {createOpen&&<CreateOrder onClose={()=>setCreateOpen(false)} onCreated={order=>{setOrders(prev=>[order,...prev.filter(value=>value.id!==order.id)]);setDetail(order);setCreateOpen(false);setSelected(order.id);notice('订单已创建')}}/>}
    ...
    {remarkEdit&&active&&<Modal title="修改卖家备注" onClose={()=>{if(!busy)setRemarkEdit(false)}}><form onSubmit={e=>void saveRemark(e)}><p>订单：{active.order_number}</p><label className="field">卖家备注<textarea rows={3} maxLength={200} value={remark} onChange={e=>setRemark(e.target.value)}/><span className="hint">显示在订单里，并打印在每页顶部（订单号后面，最多打印 60 字）。已提交订单保存后会自动重新排版并更新打印文件。</span></label>{error&&<div className="error-banner" role="alert">{error}</div>}<div className="modal-footer"><button type="button" className="button" disabled={busy} onClick={()=>setRemarkEdit(false)}>取消</button><button className="button primary" type="submit" disabled={busy}>{busy&&<Spinner/>}保存</button></div></form></Modal>}
...
function CreateOrder({onClose,onCreated}:{onClose:()=>void;onCreated:(order:CustomerOrder)=>void}){
  ...
  return <Modal title="开新订单" onClose={()=>{if(!busy)onClose()}}><form onSubmit={e=>void create(e)}><label className="field">订单号<input aria-label="订单号" required maxLength={120} autoComplete="off" value={number} onChange={e=>setNumber(e.target.value)}/><span className="hint">客户凭此号码进入订单；订单号不可重复使用。</span></label><div className="form-grid"><label className="field">可预览数量<input required type="number" min={1} max={360} value={limit} onChange={e=>setLimit(Number(e.target.value))}/></label><label className="field">最终提交数量<input required type="number" min={1} max={limit} value={finalCount} onChange={e=>setFinalCount(Number(e.target.value))}/></label><label className="field">整单重试次数<input required type="number" min={0} value={reruns} onChange={e=>setReruns(Number(e.target.value))}/></label></div><label className="field">内部备注<textarea maxLength={200} value={notes} onChange={e=>setNotes(e.target.value)} rows={3}/><span className="hint">仅工作人员可见，用于本地打印文件夹命名。</span></label>{error&&<div className="error-banner" role="alert">{error}</div>}<div className="modal-footer"><button type="button" className="button" disabled={busy} onClick={onClose}>取消</button><button className="button primary" disabled={busy||finalCount>limit}>{busy&&<Spinner/>}创建订单</button></div></form></Modal>
}

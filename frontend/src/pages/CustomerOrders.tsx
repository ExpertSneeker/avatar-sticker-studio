...
export function CustomerOrders({user,library,createRequest=0,onCreateHandled,scope='active'}:{user:User;library:CustomerLibrary;createRequest?:number;onCreateHandled?:()=>void;scope?:'active'|'history'}){
  ...
  const detail=active?<><div className="customer-staff-actions">{controls(active)}</div><div className="customer-order-meta">开单人：{active.owner_name} · 备注：{active.notes||'无'}<br/>当前水印：{active.watermark||active.owner_name}{active.delivery_ready?' · 打印文件就绪':''}{downloads[active.id]&&<p role="status">{downloads[active.id]}</p>}</div><CustomerWorkbench key={active.id} initial={active} mode="staff" library={library} onBack={()=>setSelected(null)} onChange={next=>accept({...active,...next,version:next.version??active.version})}/></>:null

...
export function CustomerOrders({user,library,createRequest=0,onCreateHandled}:{user:User;library:CustomerLibrary;createRequest?:number;onCreateHandled?:()=>void}){
  ...
  function controls(order:CustomerOrder){return <>{order.state==='cancelled'?<button className="button" disabled={busy} onClick={()=>setAction({order,path:'restore'})}>恢复订单</button>:<>{order.state==='submitted'?<><button className="button" disabled={busy||!order.delivery_ready} onClick={()=>void download([order.id])}><Download size={15}/>下载打印文件</button><button className="button" disabled={busy} onClick={()=>setAction({order,path:'unlock'})}>解锁选图</button><button className="button" disabled={busy} onClick={()=>{setPrint(order.print_settings);setAction({order,path:'repack'})}}>重新排版</button></>:null}<button className="text-button" disabled={busy} onClick={()=>setAction({order,path:'cancel'})}>取消订单</button></>}</>}
  return <div className="customer-orders">{active?<><div className="customer-staff-actions">{controls(active)}</div><div className="customer-order-meta">开单人：{active.owner_name} · 备注：{active.notes||'无'}<br/>当前水印：{active.watermark||active.owner_name}{active.delivery_ready?' · 打印文件就绪':''}{downloads[active.id]&&<p role="status">{downloads[active.id]}</p>}</div><CustomerWorkbench key={active.id} initial={active} mode="staff" library={library} onBack={()=>setSelected(null)} onChange={next=>accept({...active,...next,version:next.version??active.version})}/></>:<><div className="page-heading"><div><h1>客户订单</h1><p>开单后将订单号交给客户，也可进入订单代客户制作和选图。</p></div><div className="button-group"><a className="button" href="/guest" target="_blank" rel="noreferrer">客户入口</a><button className="button primary" onClick={()=>setCreateOpen(true)}><Plus size={16}/>开新订单</button></div></div>
    <div className="customer-order-filters"><label className="field">订单号<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="搜索订单号"/></label><label className="field">开单人<select value={owner} onChange={e=>setOwner(e.target.value)}><option value="all">全部开单人</option>{Array.from(new Map(orders.map(order=>[order.owner,order.owner_name])).entries()).map(([id,name])=><option key={id} value={id}>{name}</option>)}</select></label><label className="field">订单状态<select value={state} onChange={e=>{setState(e.target.value);setChecked([])}}><option value="all">全部有效订单</option>{Object.entries(stateLabels).map(([value,label])=><option key={value} value={value}>{value==='cancelled'?'回收站':label}</option>)}</select></label><label className="field">开始日期<input type="date" value={start} onChange={e=>setStart(e.target.value)}/></label><label className="field">结束日期<input type="date" value={end} min={start} onChange={e=>setEnd(e.target.value)}/></label><button className="button" onClick={()=>void refresh()} disabled={busy}><RefreshCw size={16}/>刷新</button></div>{dateError&&<p className="error-banner">结束日期不能早于开始日期</p>}

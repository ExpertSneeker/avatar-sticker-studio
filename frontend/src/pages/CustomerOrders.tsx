...
export function CustomerOrders({user,library,createRequest=0,onCreateHandled,scope='active'}:{user:User;library:CustomerLibrary;createRequest?:number;onCreateHandled?:()=>void;scope?:'active'|'history'}){
  ...
  return <div className="customer-orders">{active&&!history?detail:null}{(!active||history)?list:null}{active&&history&&<Modal title={'订单 '+active.order_number} onClose={()=>{if(!busy)setSelected(null)}} wide>{detail}</Modal>}

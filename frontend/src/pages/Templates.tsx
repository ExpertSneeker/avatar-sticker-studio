...
function SetEditor({set,stickers,onClose,onSaved}:{set:TemplateSet|null;stickers:Sticker[];onClose:()=>void;onSaved:()=>void}) {
 ...
 const [name,setName]=useState(set?.name||''),[code,setCode]=useState(set?.code||''),[category,setCategory]=useState(set?.category||'boy'),[busy,setBusy]=useState(false)
...
export function Templates({templates,stickers=[],admin,canEdit=admin,onRefresh}:{templates:TemplateSet[];stickers?:Sticker[];admin:boolean;canEdit?:boolean;onRefresh:()=>void}) {
  ...
  return <>
    <div className="page-heading"><div><h1>模板库</h1><p>从公共贴纸库组合有序套装，所有成员共享使用。</p></div>{canEdit&&<button className="button primary" onClick={()=>setEditor(null)}><Plus size={17}/>新建套装</button>}</div>

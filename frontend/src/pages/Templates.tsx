...
export function Templates({templates,admin,onRefresh}:{templates:TemplateSet[];admin:boolean;onRefresh:()=>void}) {
  ...
  const filtered=useMemo(()=>{
    ...
    return templates.filter(t=>(scope==='all'||(t.scope||'public')===scope)&&(category==='all'||t.category===category)&&(t.name+' '+t.code).toLocaleLowerCase().includes(query))
  ...
  return <>
    <div className="page-heading"><div><h1>模板库</h1><p>每套 12 张。公共模板共享使用，个人模板仅自己可见。</p></div><button className="button primary" onClick={()=>setEditor(null)}><Plus size={17}/>新建套装</button></div>
    <div className="filter-bar"><div className="tabs" role="group" aria-label="模板归属">{[['all','全部套装'],['public','公共模板'],['personal','个人模板']].map(([id,label])=><button className={scope===id?'active':''} aria-pressed={scope===id} onClick={()=>setScope(id)} key={id}>{label}</button>)}</div><label className="search-input"><Search size={16}/><input aria-label="搜索模板" placeholder="搜索名称或编号" value={search} onChange={e=>setSearch(e.target.value)}/></label></div>
    ...
    {filtered.length?<div className="template-library">{filtered.map(set=><article className="library-set" key={set.id}>
      ...
      <div className="library-details"><div><h3>{set.name}</h3><p>{set.code} <span>·</span> 12 张 <span>·</span> v{set.revision}</p><p>{set.scope==='personal'?'个人模板':'公共模板'} <span>·</span> {TEMPLATE_CATEGORIES.find(([id])=>id===set.category)?.[1]||set.category}</p></div><span className={'availability '+(set.active?'on':'')}>{set.active?'已上架':'未上架'}</span></div>
    ...
    </article>)}</div>:<Empty title={search||category!=='all'||scope!=='all'?'没有找到匹配的套装':'模板库等待你的第一套作品'} description="可调整筛选条件，或上传 12 张独立图片创建自己的模板套装。"><button className="button" onClick={()=>setEditor(null)}><Plus size={16}/>新建套装</button></Empty>}

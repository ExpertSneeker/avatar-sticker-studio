...
export function Admin() {
  ...
  return <><div className="page-heading"><div><h1>管理设置</h1><p>管理同时处理数量、服务接入和团队账号。</p></div></div>{error&&<div className="error-banner">{error}<button onClick={()=>void load()}>重试</button></div>}{settings?<div className="settings-layout"><form onSubmit={save}><section className="settings-section"><h2>生图服务</h2><div className="provider-heading"><strong>FAL.ai</strong><span className={'availability '+(settings.fal_configured?'on':'')}>{settings.fal_configured?'已配置密钥':'待配置'}</span></div><p className="provider-spec">GPT Image 2.5 Flare <span>·</span> Low <span>·</span> 1K <span>·</span> 透明 PNG</p><label className="field">FAL API Key<input type="password" autoComplete="new-password" value={key} onChange={e=>setKey(e.target.value)} placeholder={settings.fal_configured?'输入新密钥以替换；留空保持原值':'填写此网站使用的 API Key'}/><span className="hint">填写 FAL 密钥，无需 OpenAI 密钥。密钥仅保存在服务端，保存后不会显示原文。</span></label><label className="field">椰子抠图 API Key（按需使用）<input type="password" autoComplete="new-password" value={cutout} onChange={e=>setCutout(e.target.value)} placeholder={settings.cutout_configured?'已配置；留空保持原值':'透明背景需要修复时使用'}/></label></section><section className="settings-section"><h2>全站并发</h2><div className="form-grid"><label className="field">最大同时处理数量<input type="number" required min={1} max={40} step={1} value={settings.max_inflight} onChange={e=>setSettings({...settings,max_inflight:Number(e.target.value)})}/></label></div><p className="hint">所有账号共用上限，并同时受各账号独立上限约束，包含已提交到 FAL 排队的任务。默认 2，不限制每分钟数量。FAL 账号达到实际并发上限时会继续排队；调低此值会等待已有任务结束。多账号按当前占用均衡分配空位，占用相同时轮流执行。</p></section><section className="settings-section"><h2>固定提示词 <span className="count">v{settings.prompt_version}</span></h2><textarea rows={9} value={settings.prompt} onChange={e=>setSettings({...settings,prompt:e.target.value})}/><p className="hint">图 1 为模板，图 2 为头像。修改只影响新提交的任务。</p></section><button className="button primary" disabled={busy}>{busy?<Spinner/>:<Save size={16}/>}保存全站设置</button></form><aside><div className="aside-note"><ShieldCheck size={20}/><p>账号创建、邀请和密码重置请前往左侧「账户管理」。</p></div></aside></div>:!error&&<Spinner/>}<Maintenance/></>
}

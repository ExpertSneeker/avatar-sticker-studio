...
type DeletionPlan={preview_token:string;order_count:number;template_count:number;revision_count:number;asset_count:number;upload_count:number;blocked_count:number;can_delete:boolean}
...
export function DeleteAccountDialog({account,onClose,onDeleted}:{account:User;onClose:()=>void;onDeleted:(pendingFiles:number)=>void}) {
  ...
  return <Modal title="删除成员账号" onClose={close}>
    <p className="account-confirm-copy">将永久删除「{account.display_name}」（{account.username}）及其全部订单、个人上传和相关图片，无法恢复。公共贴纸、公共模板和其他账号的数据不受影响。</p>
    {loading?<div className="account-loading" role="status"><Spinner/>正在核对删除范围</div>:plan&&<>

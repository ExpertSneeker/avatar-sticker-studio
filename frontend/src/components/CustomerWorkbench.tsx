...
import { ArrowDown, ArrowLeft, ArrowUp, Check, ImagePlus, RefreshCw, Trash2 } from 'lucide-react'
import { api } from '../lib/api'
import { searchMatcher } from '../lib/search'
import { uploadFile } from '../lib/upload'
import { guestUpload } from '../lib/guest-upload'
import { guestApi, moveSelection, OrderMutations, quantityIds, selectionCounts, selectionError, slotBusy, stateLabels, submissionError } from '../lib/customer-orders'
...
export function CustomerWorkbench({initial,mode,library,onChange,onBack}:{initial:GuestOrder;mode:'guest'|'staff';library:CustomerLibrary;onChange?:(order:GuestOrder)=>void;onBack?:()=>void}){
  ...
  function accept(next:GuestOrder){setOrder(previous=>next.state==='cancelled'||(next.version??0)>=(previous.version??0)?next:previous);if(next.state==='cancelled'||next.state==='submitted'){setAvatars([]);setSelection([]);setComparison(null);setPicker(null);setPreflight(null);setSubmitOpen(false)}changeRef.current?.(next)}
  ...
  async function retryPending(){
    if(lock.current||!mutations.current.pendingPath)return
  ...
  return <div className={'customer-workbench '+mode+'-workbench'}>
    ...
    {mutations.current.pendingPath&&!busy&&<PendingMutation busy={!!busy} onRetry={()=>void retryPending()}/>}
    {order.state==='cancelled'?<Empty title="订单已取消" description="如需继续制作，请联系为你开单的工作人员。"/>:order.state==='submitted'?<section className="customer-submitted"><span className="submitted-check"><Check size={28}/></span><h2>已提交确认</h2><p>已选 {order.final_count} 张贴纸。需要调整请联系工作人员解锁订单。</p>{order.preview_url?<img className="customer-overview" src={order.preview_url} alt="已确认贴纸水印总览"/>:<p className="hint"><Spinner/>总览正在整理中，请稍候。</p>}</section>:order.state==='draft'?<>
      <section className="settings-section"><div className="customer-section-head"><div><h2>1. 上传头像</h2><p className="hint">每个头像分别选择模板或贴纸，最多 {order.final_count} 个头像。</p></div><label className={'button '+(busy?'disabled':'')}><ImagePlus size={16}/>添加头像<input aria-label="上传头像" type="file" accept="image/*" multiple disabled={!!busy||avatars.length>=order.final_count} onChange={e=>{void upload(e.target.files);e.target.value=''}} hidden/></label></div>{busy==='upload'&&<p role="status"><Spinner/>头像上传中 {uploadProgress}%</p>}
      ...
      <div className="notice-banner">生成后头像与贴纸组合已固定。勾选 {order.final_count} 张成品，按选中顺序排版；重跑后请确认保留哪个版本。</div>
      <div className="customer-results">{order.slots?.map((slot,index)=>{const result=slot.versions.find(v=>v.id===slot.selected_version_id),selected=selection.includes(slot.id);return <article key={slot.id} className={'customer-result '+(selected?'selected':'')}><button className="customer-result-image checker" aria-label={`查看 ${slot.sticker_code} 第 ${index+1} 张`} onClick={()=>setComparison(slot.id)}>{result?<img src={result.preview_url} alt={`${slot.sticker_code} 第 ${index+1} 张`} loading="lazy"/>:<div className="result-pending"><Status value={slot.status}/></div>}</button><div className="customer-result-body"><div><strong>{slot.sticker_code}</strong><span className="hint">头像 {(order.avatars||[]).findIndex(a=>a.id===slot.avatar_id)+1} · 第 {index+1} 张</span></div><Status value={slot.status}/>{slot.error&&<p className="error-text">{slot.error}</p>}<label className="check-line"><input type="checkbox" aria-label={`选择成品 ${index+1}`} checked={selected} disabled={!!busy||!result||!!slot.pending_version_id||slotBusy(slot)||(!selected&&selection.length>=order.final_count)} onChange={()=>toggleSlot(slot.id)}/>{selected?`第 ${selection.indexOf(slot.id)+1} 张打印`:'选为成品'}</label><button className={'button '+(slot.pending_version_id?'primary':'')} onClick={()=>setComparison(slot.id)}>{slot.pending_version_id?'对比并确认版本':'查看 / 重跑'}</button><small>重跑已用 {slot.reruns_used} / {order.rerun_limit}{slot.reruns_reserved?` · 处理中 ${slot.reruns_reserved} 次`:''}</small></div></article>})}</div>
      {!!selection.length&&<section className="settings-section customer-print-order"><h2>成品顺序</h2><p className="hint">按以下顺序制作打印文件。</p><ol>{selection.map((id,index)=>{const slot=order.slots?.find(s=>s.id===id);return <li key={id}><span>{index+1}. {slot?.sticker_code} · 第 {(order.slots?.findIndex(s=>s.id===id)??0)+1} 张</span><div><button className="icon-button" aria-label={`上移成品 ${index+1}`} disabled={!!busy||index===0} onClick={()=>setSelection(prev=>moveSelection(prev,index,-1))}><ArrowUp size={16}/></button><button className="icon-button" aria-label={`下移成品 ${index+1}`} disabled={!!busy||index===selection.length-1} onClick={()=>setSelection(prev=>moveSelection(prev,index,1))}><ArrowDown size={16}/></button><button className="icon-button" aria-label={`移除成品 ${index+1}`} disabled={!!busy} onClick={()=>toggleSlot(id)}><Trash2 size={15}/></button></div></li>})}</ol></section>}
      <div className="customer-submit-bar"><div><strong>已选 {selection.length} / {order.final_count} 张</strong><p className="hint">{finalError||'确认后将锁定选图结果并制作打印文件。'}</p></div><button className="button primary" disabled={!!busy||!!finalError} onClick={()=>setSubmitOpen(true)}>确认成品并提交</button></div>
    </>}
    {picker&&<SelectionPicker library={library} avatar={avatars.find(a=>a.upload_id===picker)!} max={order.generation_limit} onClose={()=>setPicker(null)} onSave={next=>{setAvatars(prev=>prev.map(a=>a.upload_id===picker?{...a,...next}:a));setPicker(null);setPreflight(null)}}/>}
    {preflight&&<Modal title="生成前核对" onClose={()=>{if(!busy)setPreflight(null)}}>{error&&<div className="error-banner" role="alert">{error}</div>}{mutations.current.pendingPath&&!busy&&<PendingMutation busy={!!busy} onRetry={()=>void retryPending()}/>}<div className="customer-counts"><div><span>头像</span><strong>{preflight.avatar_count}</strong></div><div><span>可选结果</span><strong>{preflight.selection_count}</strong></div><div><span>实际生成</span><strong>{preflight.generation_count}</strong></div></div><p>重复选择会保留对应份数，同一头像的相同贴纸共用首次生成结果。开始后，头像和贴纸选择将固定。</p><p className="hint">完成后需选择 {order.final_count} 张成品。</p><div className="modal-footer"><button className="button" disabled={!!busy} onClick={()=>setPreflight(null)}>返回调整</button><button className="button primary" disabled={!!busy} onClick={()=>void mutate('/generate',{avatars:avatars.map(({upload_id,template_ids,sticker_ids})=>({upload_id,template_ids,sticker_ids}))})}>{busy&&<Spinner/>}确认开始生成</button></div></Modal>}
    {compared&&<VersionComparison error={error} slot={compared} limit={order.rerun_limit} busy={!!busy} onClose={()=>setComparison(null)} pending={!!mutations.current.pendingPath&&!busy} onRetry={()=>void retryPending()} mode={mode} onRecovery={(action)=>void mutate('/slots/'+encodeURIComponent(compared.id)+'/'+action,action==='resolve'?{confirmed_ended:true}:{})} onRerun={()=>void mutate('/slots/'+encodeURIComponent(compared.id)+'/rerun')} onSelect={version_id=>void mutate('/slots/'+encodeURIComponent(compared.id)+'/select',{version_id})}/>}
    {submitOpen&&<Modal title="确认最终成品" onClose={()=>{if(!busy)setSubmitOpen(false)}}>{error&&<div className="error-banner" role="alert">{error}</div>}{mutations.current.pendingPath&&!busy&&<PendingMutation busy={!!busy} onRetry={()=>void retryPending()}/>}<p>将按已排列顺序提交 {selection.length} 张成品。提交后如需修改，请联系工作人员解锁。</p><div className="modal-footer"><button className="button" disabled={!!busy} onClick={()=>setSubmitOpen(false)}>继续检查</button><button className="button primary" disabled={!!busy||!!finalError} onClick={()=>void mutate('/submit',{slot_ids:selection})}>{busy&&<Spinner/>}确认提交</button></div></Modal>}
  </div>
}
function SelectionPicker({library,avatar,max,onClose,onSave}:{library:CustomerLibrary;avatar:DraftAvatar;max:number;onClose:()=>void;onSave:(value:AvatarChoice)=>void}){
  const [templates,setTemplates]=useState(avatar.template_ids),[stickers,setStickers]=useState(avatar.sticker_ids),[tab,setTab]=useState('templates'),[search,setSearch]=useState(''),[category,setCategory]=useState('all')
  ...
  const counts=selectionCounts([{upload_id:avatar.upload_id,template_ids:templates,sticker_ids:stickers}],library)
  return <Modal title="选择模板和贴纸" onClose={onClose} wide><div className="tabs"><button className={tab==='templates'?'active':''} onClick={()=>setTab('templates')}>模板套装</button><button className={tab==='stickers'?'active':''} onClick={()=>setTab('stickers')}>单张贴纸</button></div><div className="customer-picker-filters"><input aria-label="搜索模板或贴纸" placeholder="搜索名称或编号" value={search} onChange={e=>setSearch(e.target.value)}/><select aria-label="贴纸分类" value={category} onChange={e=>setCategory(e.target.value)}><option value="all">全部分类</option>{Array.from(new Set(source.map(s=>s.category))).map(value=><option key={value}>{value}</option>)}</select></div>{filtered.length?<div className="customer-catalog">{filtered.map(entry=>{const ids=tab==='templates'?templates:stickers;const amount=ids.filter(id=>id===entry.id).length;return <article key={entry.id} className={amount?'selected':''}><div className="customer-catalog-image">{'images' in entry?entry.images.slice(0,4).map((image,index)=><img key={index} src={image.preview_url} alt="" loading="lazy"/>):<img src={entry.preview_url} alt="" loading="lazy"/>}</div><strong>{entry.name}</strong><span>{entry.code}{'sticker_ids' in entry?` · ${entry.sticker_ids.length} 张 / 套`:''}</span><label>份数<input aria-label={`${entry.code} 份数`} type="number" min={0} max={max} value={amount} onChange={e=>{const next=quantityIds(ids,entry.id,Number(e.target.value),max);if(tab==='templates')setTemplates(next);else setStickers(next)}}/></label></article>})}</div>:<Empty title="没有匹配的模板或贴纸"/>}<div className="modal-footer"><span>{counts.selection_count} 张已选 · {counts.generation_count} 张实际生成</span><button className="button primary" disabled={counts.selection_count>max} onClick={()=>onSave({upload_id:avatar.upload_id,template_ids:templates,sticker_ids:stickers})}>应用选择</button></div></Modal>
}

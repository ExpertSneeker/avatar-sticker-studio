...
import { ArrowDown, ArrowLeft, ArrowUp, Check, ImagePlus, Minus, Plus, RefreshCw, Trash2 } from 'lucide-react'
import { api } from '../lib/api'
import { searchMatcher } from '../lib/search'
import { uploadFile } from '../lib/upload'
import { guestUpload } from '../lib/guest-upload'
import { guestApi, canApplySelection, moveSelection, OrderMutations, quantityIds, selectionCounts, selectionQuantityLimit, selectionError, slotBusy, stateLabels, submissionError } from '../lib/customer-orders'
...
export function CustomerWorkbench({initial,mode,library,onChange,onBack}:{initial:GuestOrder;mode:'guest'|'staff';library:CustomerLibrary;onChange?:(order:GuestOrder)=>void;onBack?:()=>void}){
  ...
  return <div className={'customer-workbench '+mode+'-workbench'}>
    ...
    {picker&&<SelectionPicker library={library} avatar={avatars.find(a=>a.upload_id===picker)!} others={avatars.filter(a=>a.upload_id!==picker)} max={order.generation_limit} finalCount={order.final_count} onClose={()=>setPicker(null)} onSave={next=>{const updated=avatarsRef.current.map(a=>a.upload_id===picker?{...a,...next}:a);if(!canApplySelection(selectionCounts(avatarsRef.current,library).selection_count,selectionCounts(updated,library).selection_count,order.generation_limit)){setError('已超过订单选择上限，请减少份数');return}setAvatars(updated);setPicker(null);setPreflight(null)}}/>}

...
import { X, LoaderCircle, ImagePlus, Check, Minus, Plus } from 'lucide-react'
...
export function TemplateChooser({templates,value,onChange}:{templates:TemplateSet[];value:string[];onChange:(value:string[])=>void}) {
  ...
  return <div className="template-picker">{available.map(set=><button type="button" key={set.id} className={'template-option '+(value.includes(set.id)?'selected':'')} onClick={()=>onChange(value.includes(set.id)?value.filter(id=>id!==set.id):[...value,set.id])}><div className="set-mosaic">{set.images.slice(0,4).map(im=><img key={im.id} src={im.url} alt=""/>)}</div><div><strong>{set.name}</strong><span>{set.code} · 12 张</span></div><span className="pick-check">{value.includes(set.id)?<Check size={16}/>:<Plus size={16}/>}</span></button>)}</div>
}
export function Progress({value,total}:{value:number;total:number}) {return <div className="progress-track"><span style={{width:(total?Math.min(100,value/total*100):0)+'%'}}/></div>}

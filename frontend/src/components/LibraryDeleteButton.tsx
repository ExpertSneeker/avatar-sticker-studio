...
export function LibraryDeleteButton({kind,id,code,onDeleted,iconOnly=false}:{iconOnly?:boolean;kind:'stickers'|'templates';id:string;code:string;onDeleted:()=>void}) {
  ...
  return <><button type="button" className={(iconOnly?"icon-button":"text-button")+" library-delete"} disabled={busy} aria-label={'删除'+label+' '+code} title={kind==='templates'?'删除集合，保留贴纸和历史订单':'仅可删除未被任何模板使用的贴纸'} onClick={()=>setConfirming(true)}>{busy?<Spinner/>:<Trash2 size={16}/>}{!iconOnly&&"删除"}</button>

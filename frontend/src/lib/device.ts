...
export interface SavedRecord {version:number;complete:boolean;hashes:Record<string,string>;root:FileSystemDirectoryHandle;name:string;intents?:Record<string,WriteIntent>}
...
export async function syncOrder(userId:string,orderId:string,root:FileSystemDirectoryHandle,onProgress:(message:string)=>void,options:{signal?:AbortSignal;repairModified?:boolean}={}) {
  ...
  try {
    ...
    const persist=()=>writeLocal('saved:'+key,{version:-1,complete:false,hashes:owned,intents,root,name:manifest.name})
    ...
    for(const path of plan.download) {
      ...
      onProgress('正在保存 '+(++progress)+' / '+plan.download.length)
    ...
    const record:SavedRecord={version:manifest.version,complete:manifest.complete,hashes:Object.fromEntries(manifest.files.map(f=>[f.path,f.sha256])),root,name:manifest.name}
    ...
    return typeof navigator!=='undefined'&&navigator.locks
      ? await navigator.locks.request('sticker-output:'+root.name+':'+manifest.name.normalize('NFC').toLowerCase(),{signal},run)
      : await run()

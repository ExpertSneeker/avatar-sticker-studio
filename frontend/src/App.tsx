...
import { BarChart3, ChartNoAxesCombined, House, Sticker as StickerIcon, LayoutGrid, ClipboardList, UserRound, Settings2, Plus, LogOut, X, CheckCircle2, AlertCircle, Menu, Building2 } from 'lucide-react'
...
import { directorySupported, openOrderDirectory, syncOrder } from './lib/device'
import { isAdmin, roleLabel, staffLibrary } from './lib/customer-orders'
import type { Order, Sticker, TemplateSet, User } from './lib/types'

type Page='stickers'|'workspace'|'templates'|'tasks'|'account'|'admin'|'statistics'|'global-statistics'|'users'|'organizations'
const navigation=[{id:'workspace',name:'工作台',icon:House},{id:'stickers',name:'贴纸库',icon:StickerIcon},{id:'templates',name:'模板库',icon:LayoutGrid},{id:'tasks',name:'历史订单',icon:ClipboardList},{id:'statistics',name:'统计',icon:BarChart3},{id:'account',name:'账号设置',icon:UserRound},{id:'users',name:'账户管理',icon:UserRound},{id:'organizations',name:'组织管理',icon:Building2},{id:'global-statistics',name:'全站统计',icon:ChartNoAxesCombined},{id:'admin',name:'管理设置',icon:Settings2}] as const
export default function App(){
  ...
  const [page,setPage]=useState<Page>('workspace'),[templates,setTemplates]=useState<TemplateSet[]>([]),[stickers,setStickers]=useState<Sticker[]>([]),[orders,setOrders]=useState<Order[]>([])
  const [categories,setCategories]=useState<LibraryCategory[]>(DEFAULT_CATEGORIES),[device,setDevice]=useState<Record<string,DeviceState>>({})
  const [toast,setToast]=useState<{message:string;kind:string}|null>(null),[mobileNav,setMobileNav]=useState(false),[createRequest,setCreateRequest]=useState(0),[legacyZips,setLegacyZips]=useState<string[]>([])
  const session=useRef(new AbortController()),identity=useRef<string|null>(null),authChannel=useRef<BroadcastChannel|null>(null),downloading=useRef(false)
  const updateUser=useCallback((next:User|null)=>{
    if(identity.current!==next?.id){session.current.abort();session.current=new AbortController();identity.current=next?.id||null;setOrders([]);setTemplates([]);setStickers([]);setCategories(DEFAULT_CATEGORIES);setDevice({});setLegacyZips([])}
  ...
  const refresh=useCallback(async()=>{
    ...
    try{const [sets,tasks,currentUser,assets,cats]=await Promise.all([api<TemplateSet[]>('/templates',{signal}),api<Order[]>('/orders',{signal}),api<User>('/auth/me',{signal}),api<Sticker[]>('/stickers',{signal}),api<LibraryCategory[]>('/library/categories',{signal})]);if(signal.aborted||identity.current!==userId)return;setTemplates(sets);setStickers(assets);setCategories(cats);setOrders(tasks.filter(order=>!(order as Order&{workflow_version?:number}).workflow_version||((order as Order&{workflow_version?:number}).workflow_version||0)<3));setUser(previous=>JSON.stringify(previous)===JSON.stringify(currentUser)?previous:currentUser);setError('')}catch(e){if(!signal.aborted)setError((e as Error).message)}
  ...
  async function redownload(ids:string[]){
    if(!user||downloading.current||!ids.length)return
    if(!directorySupported()){setLegacyZips(ids);return}
    const signal=session.current.signal;downloading.current=true
    try{
      const root=await window.showDirectoryPicker({mode:'readwrite',id:'legacy-print-download'})
      for(const id of ids){signal.throwIfAborted();try{const saved=await syncOrder(user.id,id,root,message=>{if(!signal.aborted)setDevice(prev=>({...prev,[id]:{message,busy:true}}))},{signal});if(!signal.aborted)setDevice(prev=>({...prev,[id]:{message:'打印文件已保存到本机',saved:true,version:saved.version}}))}catch(e){if(signal.aborted)throw e;setDevice(prev=>({...prev,[id]:{message:(e as Error).message,error:true}}))}}
    }catch(e){if((e as Error).name!=='AbortError'){notice((e as Error).message,'error');setLegacyZips(ids)}}finally{downloading.current=false}
  }
  async function openDirectory(id:string){try{await openOrderDirectory(user!.id,id)}catch(e){if((e as Error).name!=='AbortError')notice((e as Error).message,'error')}}

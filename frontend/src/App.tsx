...
import { BarChart3, ChartNoAxesCombined, House, Sticker as StickerIcon, LayoutGrid, ClipboardList, UserRound, Settings2, FolderOpen, Plus, LogOut, X, CheckCircle2, AlertCircle, Menu } from 'lucide-react'
...
type Page='stickers'|'workspace'|'templates'|'tasks'|'account'|'admin'|'statistics'|'global-statistics'|'users'
const navigation=[{id:'workspace',name:'工作台',icon:House},{id:'stickers',name:'贴纸库',icon:StickerIcon},{id:'templates',name:'模板库',icon:LayoutGrid},{id:'tasks',name:'任务中心',icon:ClipboardList},{id:'statistics',name:'统计',icon:BarChart3},{id:'account',name:'账号设置',icon:UserRound},{id:'users',name:'账户管理',icon:UserRound},{id:'global-statistics',name:'全站统计',icon:ChartNoAxesCombined},{id:'admin',name:'管理设置',icon:Settings2}] as const
export default function App() {
  ...
  const updateUser=useCallback((next:User|null)=>{
    if(identity.current!==next?.id){session.current.abort();session.current=new AbortController();identity.current=next?.id||null;setOrders([]);setTemplates([]);setStickers([]);setCategories(DEFAULT_CATEGORIES);setDevice({});setRepair(null)}
  ...
  const refresh=useCallback(async()=>{
    ...
    try{const [sets,tasks,currentUser,assets,cats]=await Promise.all([api<TemplateSet[]>('/templates',{signal}),api<Order[]>('/orders',{signal}),api<User>('/auth/me',{signal}),api<Sticker[]>('/stickers',{signal}),api<LibraryCategory[]>('/library/categories',{signal})]);if(signal.aborted||identity.current!==user.id)return;setTemplates(sets);setStickers(assets);setCategories(cats);setOrders(tasks);setUser(previous=>JSON.stringify(previous)===JSON.stringify(currentUser)?previous:currentUser);setError('')}catch(e){if(!signal.aborted)setError((e as Error).message)}
  },[user])
  ...
  const chooseDirectory=useCallback(async()=>{
    ...
    const signal=session.current.signal
    try{const handle=await pickDirectory(user.id);if(signal.aborted||identity.current!==user.id)return;setDirectory(handle);attempted.current={};notice('保存目录已选择：'+handle.name)}catch(e){if((e as Error).name!=='AbortError')notice((e as Error).message,'error')}
  },[notice,user])

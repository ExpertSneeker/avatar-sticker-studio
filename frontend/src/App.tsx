...
import { BarChart3, ChartNoAxesCombined, House, LayoutGrid, ClipboardList, ScanSearch, UserRound, Settings2, FolderOpen, Plus, LogOut, X, CheckCircle2, AlertCircle, Menu } from 'lucide-react'
...
type Page='workspace'|'templates'|'tasks'|'review'|'account'|'admin'|'statistics'|'global-statistics'|'users'
const navigation=[{id:'workspace',name:'工作台',icon:House},{id:'templates',name:'模板库',icon:LayoutGrid},{id:'tasks',name:'任务中心',icon:ClipboardList},{id:'review',name:'结果检查',icon:ScanSearch},{id:'statistics',name:'统计',icon:BarChart3},{id:'global-statistics',name:'全站统计',icon:ChartNoAxesCombined},{id:'account',name:'账号设置',icon:UserRound},{id:'users',name:'账户管理',icon:UserRound},{id:'admin',name:'管理设置',icon:Settings2}] as const
export default function App() {
  ...
  const refresh=useCallback(async()=>{
    ...
    try{const [sets,tasks,currentUser]=await Promise.all([api<TemplateSet[]>('/templates',{signal}),api<Order[]>('/orders',{signal}),api<User>('/auth/me',{signal})]);if(signal.aborted||identity.current!==user.id)return;setTemplates(sets);setOrders(tasks);setUser(previous=>JSON.stringify(previous)===JSON.stringify(currentUser)?previous:currentUser);setError('')}catch(e){if(!signal.aborted)setError((e as Error).message)}
  },[user])
